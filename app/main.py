    ...
    try:
        ...
        if latest_release:
            ...
            if assets:
                ...
                os.chdir('..') #now i am in MigrationApp folder
                version_path = os.path.join(os.getcwd(),'app')
                ...
                if latest_version != current_version:
                    ...
                    if response.status_code == 200:
                        ...
                        update_dir_path = os.path.join(os.getcwd()) #cwd: MigrationApp

                        # Move the old version to PreviousVersions folder
                        # old_version_dir = os.path.join(os.getcwd(), f"scripts")
                        # version_log_dir = os.path.join(os.path.join(os.getcwd(), 'VersionsLog'),f"scripts_{current_version}")
                        # if not os.path.exists(version_log_dir):
                        #     os.makedirs(version_log_dir)
                        # if os.path.exists(old_version_dir):
                        #     shutil.move(old_version_dir, version_log_dir)
                        ...
                        icon_path = os.path.join(os.getcwd(),'app')
                        file_path = os.path.join(os.path.join(os.getcwd(),'app'),'main.py')
                        #create the exe
                        command_to_exe = f"pyinstaller --name GinesysMigrationApp --onefile --windowed {file_path}"
